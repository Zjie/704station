...
public class DefaultAccessDecisionManager implements AccessDecisionManager {
	...
	public void decide(Authentication authentication, Object object,
		...
		if (authentication instanceof AnonymousVisitor) {
			throw new AccessDeniedException("禁止匿名用户访问");
		}
		Visitor userAu = (Visitor) authentication;
		Set<ConfigAttribute> attrs = (Set<ConfigAttribute>) configAttributes;
		String roleId = userAu.getRoleId() + "";
		// 验证该资源（url）是否能被该角色访问
		if (attrs.contains(new AuthAttribute(roleId))) {
			return;
		}
		throw new AccessDeniedException("用户未被授权");
	}
